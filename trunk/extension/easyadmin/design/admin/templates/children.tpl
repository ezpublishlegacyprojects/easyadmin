...
<div class="block">
   ...
   {/section}
    ...
    <select name="ClassID" id="classid" title="{'Use this menu to select the type of item you wish to create and click the "Create here" button. The item will be created within the current location.'|i18n( 'design/admin/node/view/full' )|wash()}">
    ...
    <input class="button" type="submit" id="standardadd" name="NewButton" value="{'Create here'|i18n( 'design/admin/node/view/full' )}" title="{'Create a new item within the current location. Use the menu on the left to select the type of the item.'|i18n( 'design/admin/node/view/full' )}" />
    <input type="hidden" name="ContentNodeID" value="{$node.node_id}" />
    <input type="hidden" name="ContentObjectID" value="{$node.contentobject_id}" />
    <input type="hidden" name="ViewMode" value="full" />
{include uri="design:easyadmin/adminadd.tpl" label="Quick add" parentnodeid=$node.node_id RedirectAfterPublish=$node.url_alias}
    </div>
    {section-else}
    <div class="left">
...
<div class="break"></div>
<a href={concat("/easyadmin/load/",$node.node_id)|ezurl()}>Load list of content</a>
</div>
